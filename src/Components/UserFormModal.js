  ...
  if (playerConut === 0 ) {
    return(
      <form onSubmit={openingSumit} className='absolute flex justify-center items-center top-0 w-screen h-screen bg-black-rgba'>
        <div className='px-4 flex flex-col justify-center items-center bg-white sm:h-[15rem] w-[15rem] h-[10rem] sm:w-[24rem]'> 
        <p className='w-[15rem] text-lg py-1'>
          Hii Guyzzzzz
        </p>
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className='absolute flex justify-center items-center top-0 w-screen h-screen bg-black-rgba'>
        <div className='px-4 flex flex-col justify-center items-center bg-white sm:h-[18rem] w-[15rem] h-[10rem] sm:w-[25rem]'> 
         ...
         <input value={user} onChange={(e)=> setUser(e.target.value)} className='text-lg w-[15rem] h-[2rem] flex items-center p-2 border-2 border-black' placeholder='PlayerO' />
        <button type='submit' className='border-2 border-black w-36 mt-6'>Add Player</button>
        </div>
    </form>
  )
}
